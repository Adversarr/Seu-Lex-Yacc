...
void Lr1::GenFollow() {
  ...
  follow_set_[p_grammar->GetAugmentedToken()].insert(p_grammar->GetEndingToken());
  ...
  while (changed) {
    ...
    for (auto&[tok, curr_follow]: follow_set_) {
      for (const auto &prod: p_grammar->GetProductions()) {
        ...
        for (auto it = prod.GetTokens().cbegin() + 1; it != prod.GetTokens().cend(); it++) {
          if (*it == tok) {
            auto rest_first = GetFirst(it + 1, prod.GetTokens().cend());
            if (rest_first.find(p_grammar->GetEpsilonToken()) != rest_first.end()) {
...
void Lr1::GenItemGo() {
  ...
  lr_item_set_.push_back(Lr1Closure(
    {LRItem(
      {p_grammar->GetAugmentedToken(), p_grammar->GetEntryToken()},
  ...
  for (IdType i = 0; i < lr_item_set_.size(); i++) {
    ...
    for_each(p_grammar->GetNonTerminators().cbegin(), p_grammar->GetNonTerminators().cend(), upd_f);
    for_each(p_grammar->GetTerminators().cbegin(), p_grammar->GetTerminators().cend(), upd_f);
  }
  stringstream ss;
  for (int i = 0; i < lr_item_set_.size(); ++i) {
    ...
    for (const auto &lri: lr_item_set_[i]) {
      ...
      ss << "[[^]]";
      for (size_t j = 1 + lri.GetCurrentPosition(); j < lri.GetTokens().size(); ++j) {
        ss << lri.GetTokens()[j] << ", ";
      }
      ss << " [";
      copy(lri.GetLookAhead().begin(),
           lri.GetLookAhead().end(),
           ostream_iterator<Token>(ss, ", "));
      ss << "] " << endl;
    }
  }
...
Lr1::TokenSet Lr1::GetFirst(vector<Token>::const_iterator first, vector<Token>::const_iterator last) const {
  if (first == last)
...
unordered_set<LRItem, LRItem::Hash> Lr1::Lr1Closure(LRItemSet lrs) const {
  ...
  while (changed) {
    ...
    for (auto &lri: lrs) {
      ...
      const auto &related_prod =  p_grammar->GetTokProdIdMap().at(next_tok.value());
      ...
      for (auto id: related_prod) {
        LRItem deferred(p_grammar->GetProductions()[id].GetTokens(), look_ahead, 0);
  ...
  for (auto &i: lrs) {
    auto it = find_if(merged.begin(), merged.end(), [&i](const LRItem &lri) {
      return lri.GetTokens() == i.GetTokens() && lri.GetCurrentPosition() == i.GetCurrentPosition();
    });
  ...
  for (IdType i = 0; i < item_go_map_.size(); ++i) {
    ...
    for (const auto &tok:  aug_terminators) {
      ...
      ParsingTable::CellTp shift_in{
        .action = ParsingTable::AutomataAction::kEmpty
      };
      if (f != item_go_map_[i].end()) {
        // 如果可以shift-in
        shift_in = {
          .action = ParsingTable::AutomataAction::kShiftIn,
          .id = f->second,
          .cause = DeferGotoCause(i, f->second, tok)
        };
      }
      
      ParsingTable::CellTp reduce{
        .action=ParsingTable::AutomataAction::kEmpty
      };
      
      for (auto &lri: curr_lrs) {
        if (lri.CanReduce() && lri.GetLookAhead().find(tok) != lri.GetLookAhead().end()) {
          ...
          if (!prod_id.has_value()) {
            ...
            copy(lri.GetTokens().cbegin(), lri.GetTokens().cend(),
                 ostream_iterator<Token>(ss, ", "));
            throw runtime_error("LRItem with token not in production list found:\n\t" + ss.str());
          }
      ...
      if (shift_in.action == ParsingTable::AutomataAction::kEmpty &&
        ...
        if (all_of(shift_in.cause.cbegin(), shift_in.cause.cend(),
                   [this, &reduce, &tok](const IdType &shift_in_prod_id) {
                     const auto &prod = p_grammar->GetProductions()[shift_in_prod_id];
...
vector<IdType> Lr1::DeferGotoCause(IdType lhs, IdType rhs, const Token &tok) {
  ...
  for (const auto &k: lr_item_set_[lhs]) {
    ...
    if (lr_item_set_[rhs].find(next_lri) == lr_item_set_[rhs].end()) // lhs -[tok]-> rhs 不成立
      continue;
